...
Plain class=${t.cls}, Encrypted class=${o} (target: match)`});let p,b=0;const T=document.querySelector("[data-e5-log]");function J(){p=i.encryptVector([3.14159265358979],"pi"),b=0;const e=i.decryptVector(p,1)[0];T.textContent=`Start value: 3.14159265358979
After encrypt/decrypt: ${e.toPrecision(14)}
Approx correct digits: ${i.preciseDigitsApprox(p)}`}J();document.querySelector("[data-e5-reset]").addEventListener("click",J);document.querySelector("[data-e5-add]").addEventListener("click",()=>{const e=i.encryptVector([.125],`plus-${b}`);p=i.add(p,e,`e5-add-${b}`),b+=1;const t=i.decryptVector(p,1)[0];T.textContent+=`
After add: ${t.toPrecision(14)} | digits~${i.preciseDigitsApprox(p)}`});document.querySelector("[data-e5-mul]").addEventListener("click",()=>{const e=i.encryptVector([1.1],`mul-${b}`);p=i.rescale(i.multiply(p,e,`e5-mul-${b}`),`e5-rescale-${b}`),b+=1;const t=i.decryptVector(p,1)[0];T.textContent+=`
